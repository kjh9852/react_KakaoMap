...
const SearchForm = (props) => {
  ...
  const mapSearchHandler = async (enteredInputValue) => {
    try {
      await fetch(
        `https://dapi.kakao.com/v2/local/search/address.json?query=${enteredInputValue}`,
        {
          body: JSON.stringify(),
          headers: {
        ...
        .then(async (res) => {
          ...
          if (documents.length === 0) {
            try {
              await fetch(
                `https://dapi.kakao.com/v2/local/search/keyword.json?y=${geoCoord.y}&x=${geoCoord.x}&radius=20000&query=${enteredInputValue}`,
                {
                  body: JSON.stringify(),
                  headers: {
                    Authorization: `KakaoAK ${token}`,
